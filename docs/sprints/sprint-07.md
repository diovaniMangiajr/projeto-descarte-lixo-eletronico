...
| \#28 | \[TASK\] Implementação da lógica de listagem e moderação de Feedbacks (Update status) | Média | **Pendente / Adiado** |
...
* **docs/testes/plano-de-testes.md:** Documentação oficial atualizada (V2), consolidando a estratégia de JUnit, Mocks, Testcontainers, API Cat e a matriz simplificada de rastreabilidade para homologação técnica interna.  
...
* Plano de testes estratégicos estruturado e revisado com foco em homologação interna (V2).  
* Zera dos débitos técnicos de mapas, listagem pública de pontos e modal detalhado em React.  
* Adicionados validadores de campos obrigatórios no DTO de feedbacks públicos do cidadão.

**O que ficou pendente (Débito Técnico para a Sprint 8):**

* A issue **\#28** (Listagem e moderação administrativa de feedbacks) foi transferida para a próxima sprint devido ao esforço de tempo concentrado na integração de mapas e amarração da arquitetura técnica de testes.  
...
4. **Finalização de Escopo:** Concluir a tela administrative pendente de moderação de feedbacks (\#28).
